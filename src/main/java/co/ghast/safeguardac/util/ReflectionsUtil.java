...
public class ReflectionsUtil {
    ...
    public static boolean canDestroyBlock(Player player, Block block) {
        ...
        return (boolean) getMethodValue(getMethod(getNMSClass("PlayerInventory"), "b", getNMSClass("Block")), inventory, SafeGuard.getInstance().getVersionUtil().bukkitVersion().contains("1.8.9") ? getBlockData(block) : getVanillaBlock(block));
    }
    ...
    public static Object getBlockPosition(Location location) {
        if (SafeGuard.getInstance().getVersionUtil().bukkitVersion().contains("1.8")) {
    ...
    public static BoundingBox getBlockBoundingBox(Block block) {
        try {
            if (!isBukkitVerison("1_7")) {
                ...
                if (SafeGuard.getInstance().getVersionUtil().bukkitVersion().contains("1.13")) {
                    if (!isNewVersion()) {
    ...
    public static boolean isBukkitVerison(String version) {
        return SafeGuard.getInstance().serverVersion.contains(version);
    }
